...
public class PossibilitySignUpIntoSystemFireFox {
	...
	@BeforeGroups(groups = { EXISTENCE_OF_SIGNUP, FIRST_NAME_VALIDATION, LAST_NAME_VALIDATION, EMAIL_VALIDATION,
			PASSWORD_VALIDATION, PASSWORD_CONFIRM_VALIDATION, PASSWORD_CONFIRM, SIGNUP_FUNCTIONALITY })
	public void beforeExistenceOfSignUp() {
		FirefoxDriverManager.getInstance().setup();
		driver = new FirefoxDriver();
		driver.get("https://green-tourism.herokuapp.com");
		driver.manage().window().maximize();
		page = new HeadForGuestUserPage(driver).clickLeftNavButton().clickSignUp();
	}

	@AfterGroups(groups = { EXISTENCE_OF_SIGNUP, FIRST_NAME_VALIDATION, LAST_NAME_VALIDATION, EMAIL_VALIDATION,
			PASSWORD_VALIDATION, PASSWORD_CONFIRM_VALIDATION, PASSWORD_CONFIRM, SIGNUP_FUNCTIONALITY })
	...
	public void checkPasswordToLess8s() {
		...
		Assert.assertEquals(actual, "Password should be longer than 8 characters",
				"Error: system don't show error message for invalid data");
	}
	...
	public void checkConfirmPasswordToLess8s() {
		...
		String actual = page.setPasswordConfirm(LESS_8_PASSWORD).disableFocus().getPasswordConfirmErrorMessage()
				.getText();
		Assert.assertEquals(actual, "Passwords doesn't match",
				"Error: system don't show error message for invalid data");
	}
	...
	@Test(groups = { PASSWORD_CONFIRM }, dependsOnMethods = { "checkExistencePasswordInput",
			"checkExistenceConfitmPasswordInput" })
	public void confirmPassword() {
		String actual = page.setPassword(PASSWORD_8).setPasswordConfirm(PASSWORD_8).disableFocus()
				.getPasswordConfirmErrorMessage().getText();
		Assert.assertEquals(actual, "", "Error: system shows error message for valid data");
	}
	...
	public void confirmPasswordNeg() {
		String actual = page.setPassword(PASSWORD_8).setPasswordConfirm(PASSWORD_9).disableFocus()
				.getPasswordConfirmErrorMessage().getText();
		Assert.assertEquals(actual, "Passwords doesn't match",
				"Error: system don't show error message for invalid data");
	}
	...
	public void clearSignUpForm() {
		page.clearFirstName().clearLastName().clearEmail().clearPassword().clearPasswordConfirm();
	}

	@Test(groups = { SIGNUP_FUNCTIONALITY }, dependsOnMethods = { "checkExistenceSignUpForm",
			"checkExistenceFirstNameInput", "checkExistenceLastNameInput", "checkExistenceEmailInput",
			"checkExistencePasswordInput", "checkExistenceConfitmPasswordInput" })
	public void checkSignUp() {
		String expected = "https://green-tourism.herokuapp.com/#!/profile";
		HeadForLoggedUserPage headForLoggedUserPage = page.setFirstName(nameGenerator()).setLastName(nameGenerator())
				.setEmail(emailGenerator()).setPassword(PASSWORD_8).setPasswordConfirm(PASSWORD_8).clickSignUpButton();
